...
pub fn process_repay(ctx: Context<Repay>, amount: u64) -> Result<()> {
    ...
    match ctx.accounts.mint.to_account_info().key() {
        ...
        _ => borrow_value = user.borrowed_sol,
    }
    let time_diff = Clock::get()?.unix_timestamp - user.last_updated_borrowed;

    let bank = &mut ctx.accounts.bank;

    let accrued_interest = (bank.total_borrowed as f64
        * (E.powf(bank.interest_rate as f32 * time_diff as f32) as f64))
        as u64;

    bank.total_borrowed = bank.total_borrowed.saturating_sub(accrued_interest);

    let value_per_share = bank.total_borrowed as f64;
    ...
    let cpi_prpgram = ctx.accounts.token_program.to_account_info();

    let cpi_ctx = CpiContext::new(cpi_prpgram, transfer_cpi_accounts);
